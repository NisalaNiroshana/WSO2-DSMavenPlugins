...
package org.wso2.maven.dslayout.artifact.utils;
...
public class FileManagementUtils {
    ...
    private static final String DSGADGET_TEMP = "dsglayout-tmp";
    ...
    public static File createArchive(File location, File artifactLocation, String artifactName) throws Exception {
        ...
        File layoutDataFolder = new File(targetFolder, DSGADGET_TEMP);
        if (!layoutDataFolder.mkdirs()) {
            logger.error(ERROR_CREATING_CORRESPONDING_ZIP_FILE);
        }
        File zipFolder = new File(layoutDataFolder, artifactLocation.getName());
        if (!zipFolder.mkdirs()) {
            logger.error(ERROR_CREATING_CORRESPONDING_ZIP_FILE);
        }
        ;
        ...
        FileUtils.deleteDirectory(layoutDataFolder);
        return zipFile;
    }
